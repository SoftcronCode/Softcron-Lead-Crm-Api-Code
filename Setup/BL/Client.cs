...
namespace Setup.BL
{
    public class Client : IClient
    {
        ...
        public Client(IHostEnvironment hostEnvironment, IAppVariables appVariables, IHttpContextAccessor httpContextAccessor, IConfiguration con)
        {
        ...
        #endregion
        #region Method
        public ResponseClass<BO.AddClientResponse> AddClient(AddClientDTO ObjMaster)
        {
            ResponseClass<BO.AddClientResponse> response = new ResponseClass<BO.AddClientResponse>();
            #region Validation
            if (ObjMaster == null)
            {
           ...
           // _appVariables.SetAppVariables();
            ...
            {
                #region Parameters
                objSpParameters.Add("SPClientName", DbType.String, ObjMaster.ClientName, ParameterDirection.Input);
                objSpParameters.Add("SPMobileNumber", DbType.String, ObjMaster.MobileNumber, ParameterDirection.Input);
                objSpParameters.Add("SPEmailID", DbType.String, ObjMaster.EmailID, ParameterDirection.Input);
                ...
                var objAddRes = new BO.AddClientResponse();
               
                objCommandExecutor.ExecuteDataSet(CommandType.StoredProcedure, "client_master_insert", DSMaster, TableName, objSpParameters);

                response.responseObject = response.responseObject ?? new List<BO.AddClientResponse>();
                if (DSMaster != null && DSMaster.Tables.Count > 0 && DSMaster.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        objAddRes.ClientMasterID = Convert.ToInt32(DSMaster.Tables[0].Rows[0]["ClientMasterID"]);
                        response.responseObject.Add(objAddRes);
